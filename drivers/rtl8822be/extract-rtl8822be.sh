...
# Seed ONLY from what is actually loaded right now, and ONLY from the rtw88
# family. Do NOT glob 'rtw*' on disk: that also matches the unrelated rtw89
# family (often present as /updates/dkms/rtw89_*_git.ko.xz from another DKMS
# package) and would drag a dozen useless modules into the bundle, where they
# would shadow the target system's modules from the higher-priority updates/.
mapfile -t SEED_NAMES < <(lsmod | awk 'NR>1 && $1 ~ /^rtw88/ {print $1}')
[[ ${#SEED_NAMES[@]} -gt 0 ]] || die "no loaded rtw88 module - driver not running?"

SEEDS=()
for n in "${SEED_NAMES[@]}"; do
    f="$(find "${MODROOT}" -type f -name "${n}.ko*" -print -quit 2>/dev/null || true)"
    [[ -n ${f} ]] && SEEDS+=("${f}")
done
...
while [[ ${#queue[@]} -gt 0 ]]; do
    ...
    for dep in $(modinfo -F depends "${ko}" 2>/dev/null | tr ',' ' '); do
        dep="${dep%.ko}"
        # Stay inside the rtw88 family. Things like mac80211/cfg80211 are
        # stock kernel modules that the target box already has; copying them
        # only bloats the payload and risks clobbering a good file.
        [[ ${dep} == rtw88* ]] || continue
